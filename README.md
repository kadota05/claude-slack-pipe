...
- **PCがスリープするとプロセスが止まります。** デフォルトでは `caffeinate -s` を使って起動するため、PCの蓋を開いた状態であればアイドルスリープを防止できます（プロセス終了時に自動解除）。蓋を閉じた状態でも使いたい場合は `sudo pmset -a disablesleep 1` が必要です。これはシステム設定の変更なので、不要になったら `sudo pmset -a disablesleep 0` で手動で戻してください。電車でPCを閉じてカバンに入れたまま使いたい、といった場面が多い場合はこの設定をおすすめします
