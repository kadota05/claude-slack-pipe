...
- **PC側のWi-Fiを切り替えるとプロセスの再起動が必要です。** SlackとPCはWebSocketで接続しているため、PC側のWi-Fiの切り替え（家のWi-Fi→テザリング、2階のWi-Fi→1階のWi-Fiなど）でIPアドレスが変わると接続が切れます。その場合はSlackで `cc /restart-bridge` と送信して再起動してください。スマホ側のWi-Fi切り替えは接続に影響しません（Slackのサーバーとスマホの接続はSlack側が管理しているためです）。
