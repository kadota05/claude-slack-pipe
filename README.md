...
- **PC側のWi-Fiが切れると自動で再接続します。** 家のWi-Fi→テザリング、2階→1階など、Wi-Fiが変わるとWebSocket接続が切れますが、macOSの`scutil`でネットワーク変化を自動検知し、Bridgeが自動再起動します。Wi-Fi切断中に送ったメッセージは無視されます（復帰後に再送してください）。スマホ側のWi-Fi切り替えは接続に影響しません。


## 対応済み（2026-03-19）
- Wi-Fi切り替え時の自動再接続 — macOSの`scutil`でネットワーク変化を検知し、WiFi復帰時にBridgeを自動再起動
- localhostをBridge起動PC外（スマホなど）から閲覧可能にする — cloudflaredトンネル経由でSlackリンクを自動変換
