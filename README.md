...
- **PC側のWi-Fiを切り替えるとプロセスの再起動が必要です。** SlackとはWebSocketで接続しているため、PC側のWi-Fiの切り替え（家のWi-Fi→テザリング、2階のWi-Fi→1階のWi-Fiなど）でIPアドレスが変わると接続が切れます。その場合はプロセスを再起動してください。スマホ側のWi-Fi切り替えは影響しません。将来的には自動再接続に対応したいと考えています。
