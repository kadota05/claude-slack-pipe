...
### 5. ユーザー通知（復帰時に1メッセージで完結）
WiFi切断時にSlack通知を送ることは物理的に不可能（scutil通知時点で既にIPなし）。そのため、復帰時に1メッセージで切断と復帰を伝える設計にした。

復帰時: 直近10分以内のアクティブセッションに「🔄 PCのWiFiが切断されたため一時停止していました。再接続を検知したのでBridgeを再起動しています...」→ 再起動完了後「✅ Bridgeの再起動が完了しました」に更新。
...
- **`scutil`の出力は`changedKey`（スペースなし）。** `changed key`ではない。ドキュメントに記載がなく、実機テストでしか判明しなかった。
- **WiFi切断中のSlack通知は物理的に不可能。** scutilが通知する時点で既にIPアドレスが失われている。切断通知と復帰通知を分ける設計は成り立たず、復帰時に1メッセージで完結させるのが正解。
