...
1. **Bridge起動時** → ハートビート書き込み開始 + 全ユーザーのホームタブをActiveに更新
...
4. **正常終了時**（SIGINT, SIGTERM）→ ホームタブをInactiveに更新 + ハートビートファイルを削除
...
4. 各候補のJSONL先頭を読み、最初の `type: "human"` エントリを探す（最大20行まで読み取り、見つからなければスキップ）
5. 定期実行フィルタを適用（後述）
6. 残った上位5件を表示
...
- JSONLファイルは**先頭最大20行のみ**読み取る（最初の `type: "human"` エントリが見つかれば打ち切り）
- `type: "human"` が20行以内に見つからないセッションはスキップ（空セッションやシステムのみのセッション）
