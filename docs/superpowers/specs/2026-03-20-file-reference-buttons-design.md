...
```json
{
  ...
  "accessory": {
    ...
    "action_id": "view_file_content:src/index.ts",
    "value": "src/index.ts"
  }
}
```
...
buildFileReferenceBlocks(filePaths) を追加
  ↓
ボタンクリック時:
  action_id = "view_file_content:{path}"
...
- **mrkdwn変換後処理**: localhost URL書き換えと同じレイヤー（mrkdwn変換後）でパス検出を行う（知見: Bug1回避）
