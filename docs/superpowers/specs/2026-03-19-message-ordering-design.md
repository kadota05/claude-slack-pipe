...
100文字閾値によるバッファリングは廃止。ただし、subagent実行中のみテキストをバッファする（詳細はSection 4のCase 1参照）。

**処理フロー:**

```
テキストイベント到着:
  subagent実行中? (activeSubagents.size > 0)
    → YES: テキストをバッファ（Section 4 Case 1）
    → NO:
      1. activeGroupがあれば → completedGroupsに移動
      2. activeBundleがあれば → collapse (updateで折りたたみ表示に)
...
live状態のbundleメッセージ更新時は、`activeGroup`（thinking/tool）と全`activeSubagents`の両方からブロックを構築する。

```
buildLiveBundleBlocks():
  blocks = []
  // 1. completedGroups（折りたたみ済み）
  for (cg of activeBundle.completedGroups):
    blocks.push(buildCompletedGroupBlock(cg))
  // 2. activeGroup（thinking/tool、あれば）
  if (activeGroup):
    blocks.push(buildActiveGroupBlock(activeGroup))
  // 3. activeSubagents（全て）
  for ([id, agent] of activeSubagents):
    blocks.push(buildSubagentBlock(agent))
  return blocks
```
...
Case 1: subagent実行中にテキスト
  → テキストはGroupTracker内のpendingTextBufferに蓄積
  → handleSubagentCompleteでactiveSubagents.size === 0になった時点で:
    1. activeGroupをcompletedに移動（あれば）
    2. bundle collapse
    3. pendingTextBufferの内容でBundleAction(postMessage)を返す
    4. pendingTextBufferをクリア
...
| 並列subagent×3 + 3ラウンド | 5 postMessage | 7 postMessage | subagentは同一bundle内なのでpostMessage数は増えない |
